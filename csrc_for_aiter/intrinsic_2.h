...
template <typename T>
__forceinline__ __device__ void buffer_load_reg_dwordx2(const T *ptr, vec<int, 2> &rsrc, const int vindex, int offset)
{ // const int offset
    __builtin_amdgcn_sched_barrier(0);
    intx4 global_ptr;
...
template <typename T>
__forceinline__ __device__ void buffer_load_reg_dwordx2_w4a8(const T *ptr, vec<int, 2> &rsrc, const int s_offset, int v_offset)
{
    __builtin_amdgcn_sched_barrier(0);
    intx4 global_ptr;
    *(uint64_t *)&global_ptr = reinterpret_cast<uint64_t>(ptr);
    ...
    asm volatile("buffer_load_dwordx2 %0, %1, %2, %3 offen offset:0 \n"
                 : "=v"(rsrc)
...
// buferr_load_reg
template <typename T>
__forceinline__ __device__ void buffer_load_reg_dwordx4(const T *ptr, vec<int, 4> &rsrc, const int vindex, int offset)
{ // const int offset
    __builtin_amdgcn_sched_barrier(0);
    intx4 global_ptr;
    *(uint64_t *)&global_ptr = reinterpret_cast<uint64_t>(ptr); // res[0]放首地址信息
...
// buferr_load_reg
template <typename T>
__forceinline__ __device__ void buffer_load_reg_dwordx4_w4a8(const T *ptr, vec<int, 4> &rsrc, const int s_offset, int v_offset)
{ // const int offset
    __builtin_amdgcn_sched_barrier(0);
    intx4 global_ptr;
    *(uint64_t *)&global_ptr = reinterpret_cast<uint64_t>(ptr); // res[0]放首地址信息
...
template <typename T>
__forceinline__ __device__ void buffer_load_reg_dwordx4_uint32(const T *ptr, vec<uint32_t, 4> &rsrc, const int vindex, int offset)
{ // const int offset
...
template <typename T>
__forceinline__ __device__ void buffer_load_reg_dwordx4_gls_slc(const T *ptr, vec<float, 4> &rsrc, const int vindex, int offset)
{ // const int offset
...
// intx4 v4i;
// __builtin_amdgcn_sched_barrier(0);
#if defined(__gfx936__) || defined(__gfx928__) || defined(__gfx92a__) || defined(__gfx938__)
...
template <class Element>
__device__ vec4_fp32 mmac_fp8_lit(const vec<Element, 8> &v1, const vec<Element, 8> &v2, vec4_fp32 &v3)
{
#if defined(__gfx938__)
    v3 = __builtin_hcu_mmac_f32_16x16x32_fp8_fp8_lit_lts(v1, v2, v3, 1, 0);
#else
    v3 = {0, 0, 0, 0};
#endif
...
__host__ __device__ static uint8_t __float2e4m3(const float src)
{
...
__host__ __device__ static float __e4m32float(const uint8_t src)
{
