...
#define int4_moe_wna16_cal(n_loop)                                                                                                                                                        \
  ...
  {                                                                                                                                                                                       \
    ...
    { /*stage 1 load data*/                                                                                                                                                               \
      ...
      int offset_n = blockIdx.y * BLOCK_SIZE_N_LOOPS * BLOCK_SIZE_N + n_loop * BLOCK_SIZE_N + w_lane_n_idx;                                                                               \
      int k = (blockIdx.x * BLOCK_SIZE_K_LOOPS + k_loop) * BLOCK_SIZE_K + w_lane_k_idx * 8;                                                                                               \
      ...
      {                                                                                                                                                                                   \
        ...
        {                                                                                                                                                                                 \
          int w_warp_buffer_load_lds_offset = warp_id * (WARP_N * 128) + load * (4 * 128);                                                                                                \
          int gvOffset_s = 0;                                                                                                                                                             \
          lds_stage_offset = stage_id * (BLOCK_SIZE_N * BLOCK_SIZE_K / 8);                                                                                                                \
  ...
  { /*stage 1 load data*/                                                                                                                                                                 \
    constexpr int W_LOAD_REQUESTS = WARP_N / 4;                                                                                                                                           \
    int offset_n = blockIdx.y * BLOCK_SIZE_N_LOOPS * BLOCK_SIZE_N + (n_loop + 1) * BLOCK_SIZE_N + w_lane_n_idx;                                                                           \
    int k = (blockIdx.x * BLOCK_SIZE_K_LOOPS + k_loop) * BLOCK_SIZE_K + w_lane_k_idx * 8;                                                                                                 \
    const int32_t weight_offset = offset_n * size_k + k;                                                                                                                                  \
                                                                                                                                                                                          \
    {                                                                                                                                                                                     \
    ...
    {                                                                                                                                                                                     \
      {                                                                                                                                                                                   \
        ...
        { /*k direction,K_TILE=64, (t0,t16,t32,t48) load 16 int8 data per loop*/                                                                                                          \
          int lds_offset = (warp_id * (WARP_N * 128) + (((lane_id & 15) * 128)) + j * 32 + (lane_id / 16) * 8) / pack_factor + stage_id * (BLOCK_SIZE_N * K_TILE / 8);                    \
...
__global__ void __launch_bounds__(256, 1) moe_wna16_gemm_kernel_2(
...
template <typename scalar_t,
          ...
          bool USE_ATOMIC,
          int THREADS_PER_BLOCK>
void run_moe_wna16_gemm_2(const scalar_t *input,
                          float *output_fp32,
                          ...
                          int size_k)
{
  ...
  auto kernel = moe_wna16_gemm_kernel_2<scalar_t, bit, top_k, BLOCK_SIZE_M, BLOCK_SIZE_N, BLOCK_SIZE_K, has_zp, mul_topk_weight, group_size, BLOCK_SIZE_M_LOOPS, BLOCK_SIZE_N_LOOPS, BLOCK_SIZE_K_LOOPS, USE_ATOMIC, THREADS_PER_BLOCK>;
  constexpr int K_TILE = 128;
