...
namespace W4A16_UTILS
{
    ...
    namespace DEQUANTER
    {
        __device__ inline float fp4_e2m1_to_float(uint32_t code)
        {
            uint32_t mag = code & 0x7;
            float val = 0.0f;
            if (mag == 1)
            {
                val = 0.5f;
            ...
            return (code & 0x8) ? -val : val;
        }

        template <typename T>
        __forceinline__ __device__ uint16_t fp4_num_as_u16_bits(T value)
        {
        ...
        template <typename Element, typename scalar_t2>
        __device__ inline void dequant_fp4_e2m1_scaled(uint32_t q, float scale, scalar_t2 *res)
        {
            uint32_t *out = reinterpret_cast<uint32_t *>(res);
#pragma unroll
            for (int pack_id = 0; pack_id < 4; pack_id++)
            {
                uint32_t code0 = (q >> (pack_id * 4)) & 0x0f;
                uint32_t code1 = (q >> (pack_id * 4 + 16)) & 0x0f;
                Element v0 = b32_to_b16<Element>(fp4_e2m1_to_float(code0) * scale);
                Element v1 = b32_to_b16<Element>(fp4_e2m1_to_float(code1) * scale);
                out[pack_id] = static_cast<uint32_t>(fp4_num_as_u16_bits(v0)) |
                               (static_cast<uint32_t>(fp4_num_as_u16_bits(v1)) << 16);
            }
        }

        template <typename Element>
        __forceinline__ __device__ uint16_t fp4_e2m1_e8m0_to_bits(uint32_t code, uint8_t scale)
        {
            const uint32_t mag = code & 0x7;
            const uint32_t sign = (code & 0x8) ? 0x8000 : 0;
            if (mag == 0)
            {
            ...
            const bool has_half_mant = (mag & 1) && mag != 1;
            if constexpr (std::is_same_v<Element, __hip_bfloat16>)
            {
                const int exp = static_cast<int>(scale) + exp_delta;
                if (exp <= 0)
                {
                    return static_cast<uint16_t>(sign);
                }
                if (exp >= 255)
                {
                    return static_cast<uint16_t>(sign | 0x7f80);
                }
                return static_cast<uint16_t>(
                    sign | (static_cast<uint32_t>(exp) << 7) | (has_half_mant ? 0x40 : 0));
            }
            else
            {
                const int exp = static_cast<int>(scale) - 112 + exp_delta;
                if (exp <= 0)
                {
                    return static_cast<uint16_t>(sign);
                }
                ...
                return static_cast<uint16_t>(
                    sign | (static_cast<uint32_t>(exp) << 10) | (has_half_mant ? 0x200 : 0));
            }
        }

        template <typename Element, typename scalar_t2>
        __forceinline__ __device__ void dequant_fp4_e2m1_e8m0(uint32_t q, uint8_t scale, scalar_t2 *res)
        {
            uint32_t *out = reinterpret_cast<uint32_t *>(res);
#pragma unroll
            for (int pack_id = 0; pack_id < 4; pack_id++)
            {
                const uint32_t code0 = (q >> (pack_id * 4)) & 0x0f;
                const uint32_t code1 = (q >> (pack_id * 4 + 16)) & 0x0f;
                const uint16_t v0 = fp4_e2m1_e8m0_to_bits<Element>(code0, scale);
                const uint16_t v1 = fp4_e2m1_e8m0_to_bits<Element>(code1, scale);
                out[pack_id] = static_cast<uint32_t>(v0) | (static_cast<uint32_t>(v1) << 16);
            }
        }

        __forceinline__ __device__ float e8m0_to_float_fast(uint8_t scale)
        {
            union
            {
                uint32_t u;
        ...
        __forceinline__ __device__ uint32_t pack_bf16_pair_from_f32(float lo, float hi)
        {
#if defined(__gfx938__)
            uint32_t packed;
            ...
            return packed;
#else
            const __hip_bfloat16 lo_b = __float2bfloat16(lo);
            const __hip_bfloat16 hi_b = __float2bfloat16(hi);
        ...
        template <typename Element, typename scalar_t2>
        __forceinline__ __device__ void dequant_fp4_e2m1_e8m0_lut(
            uint32_t q, uint8_t scale, const float *fp4_lut, scalar_t2 *res)
        {
            if constexpr (!std::is_same_v<Element, __hip_bfloat16>)
            {
                dequant_fp4_e2m1_e8m0<Element, scalar_t2>(q, scale, res);
            }
            else
            {
                uint32_t *out = reinterpret_cast<uint32_t *>(res);
                const float scale_f = e8m0_to_float_fast(scale);
#pragma unroll
                for (int pack_id = 0; pack_id < 4; pack_id++)
                {
                    const uint32_t code0 = (q >> (pack_id * 4)) & 0x0f;
                    const uint32_t code1 = (q >> (pack_id * 4 + 16)) & 0x0f;
                    const float v0 = fp4_lut[code0] * scale_f;
                    const float v1 = fp4_lut[code1] * scale_f;
                    out[pack_id] = pack_bf16_pair_from_f32(v0, v1);
                }
            }
        }

        __device__ inline float e8m0_to_float(uint8_t scale)
        {
            if (scale == 0)
            {
                return 0x1p-127f;
    ...
    namespace SCALEHELPER
    {
        ...
        template <>
        struct vec_scale<half_t, 3>
        {
            using type = scale_array<half_t, 3>;
        };

        template <>
        struct vec_scale<bhalf_t, 3>
        {
            using type = scale_array<bhalf_t, 3>;
        };

        template <>
        struct vec_scale<half_t, 6>
        {
            using type = scale_array<half_t, 6>;
        };

        template <>
        struct vec_scale<bhalf_t, 6>
        {
            using type = scale_array<bhalf_t, 6>;
        };
...
#define WFP4A16_STAGE1_SCALE_SWITCH(size_k, ...) \
    [&] {                                                       \
  if (size_k == 192) {                                   \
    constexpr static int32_t PACK_SCALES = 3;                  \
    return __VA_ARGS__();                                   \
  }else if (size_k == 256) {                                   \
    constexpr static int32_t PACK_SCALES = 4;                  \
    return __VA_ARGS__();                                   \
  }else if (size_k == 384) {                                   \
    constexpr static int32_t PACK_SCALES = 6;                  \
    return __VA_ARGS__();                                   \
  }else if (size_k == 768) {                                   \
    constexpr static int32_t PACK_SCALES = 6;                  \
    return __VA_ARGS__();                                   \
...
template <
    ...
    int32_t PACK_SCALES,
    bool FP4_E2M1 = false,
    bool FP4_A_LOAD_BY_WARP = false,
    bool FP4_K_SPLIT = false>
__forceinline__ __device__ void gemm_nt_wfp4a16_stage1_b128_k64_marlinS_scale_ldx(
    const Element *input_ptr,
    const uint32_t *weight_ptr,
    uint32_t *weight_zeros_ptr,
    Element *weight_scale_ptr,
    ...
    const int warp_m_id)
{
    ...
    constexpr int MFMA_N = 16;
    constexpr int warp_n_num = BLOCK_N / WARP_N; // 4
    constexpr int warp_k_num = FP4_K_SPLIT ? (BLOCK_K / WARP_K) : 1;
    constexpr int COMPUTE_K = FP4_K_SPLIT ? WARP_K : BLOCK_K;
    int warp_k_id = FP4_K_SPLIT ? (warp_id % warp_k_num) : 0;
    int warp_n_id = FP4_K_SPLIT ? ((warp_id / warp_k_num) % warp_n_num)
                                : (warp_id % warp_n_num);

    const uint8_t *g_expert_qzero = reinterpret_cast<const uint8_t *>(weight_zeros_ptr);

    extern __shared__ float lds_buffer[];
    constexpr int32_t FP4_K_SPLIT_A_SLICE = BLOCK_M * COMPUTE_K;
    constexpr int32_t FP4_K_SPLIT_B_SLICE = WARP_N * COMPUTE_K;
    Element *lds_A = FP4_K_SPLIT ? reinterpret_cast<Element *>(lds_buffer) + warp_id * FP4_K_SPLIT_A_SLICE : reinterpret_cast<Element *>(lds_buffer);
    Element *lds_B = FP4_K_SPLIT ? reinterpret_cast<Element *>(lds_buffer) +
                                       WARP_NUM * FP4_K_SPLIT_A_SLICE + warp_id * FP4_K_SPLIT_B_SLICE
                                 : reinterpret_cast<Element *>(lds_buffer);
    constexpr int32_t FP4_LUT_BYTES = 64;
    constexpr int32_t FP4_LUT_FLOATS = FP4_LUT_BYTES / sizeof(float);
    constexpr int32_t FP4_LUT_OFFSET_FLOATS =
        FP4_K_SPLIT ? (WARP_NUM * (FP4_K_SPLIT_A_SLICE + FP4_K_SPLIT_B_SLICE) * sizeof(Element) +
                       ((BLOCK_M / WARP_M) * (BLOCK_N / WARP_N) *
                        (BLOCK_K / WARP_K - 1) * WARP_M * (WARP_N + 1) * sizeof(float)) +
                       sizeof(float) - 1) /
                          sizeof(float)
                    : (16 * 1024) / sizeof(float);
    float *fp4_lut = lds_buffer + FP4_LUT_OFFSET_FLOATS;

    constexpr int32_t BLOCK_SIZE = WARP_NUM * 64;
    constexpr int32_t ELEMENTS_PER_LD_A = 128 / (sizeof(Element) * 8 /*bit*/);
    constexpr int32_t G2R_A_COLS = COMPUTE_K / ELEMENTS_PER_LD_A;
    constexpr int32_t G2R_A_ROWS = BLOCK_SIZE / G2R_A_COLS;
    constexpr int32_t FP4_A_ROWS_PER_WARP = 16;
    constexpr int32_t FP4_A_ROWS_PER_LD_ROUND = 64 / G2R_A_COLS;
    constexpr int32_t FP4_A_LD_ROUNDS_PER_GROUP =
        (FP4_A_ROWS_PER_WARP + FP4_A_ROWS_PER_LD_ROUND - 1) / FP4_A_ROWS_PER_LD_ROUND;
    constexpr int32_t FP4_A_LOAD_GROUPS =
        (BLOCK_M + FP4_A_ROWS_PER_WARP - 1) / FP4_A_ROWS_PER_WARP;
    constexpr int32_t FP4_A_LOAD_WARPS = FP4_K_SPLIT ? (WARP_NUM / warp_k_num) : WARP_NUM;
    constexpr int32_t FP4_A_GROUPS_PER_WARP =
        (FP4_A_LOAD_GROUPS + FP4_A_LOAD_WARPS - 1) / FP4_A_LOAD_WARPS;
    constexpr int32_t LEGACY_G2S_LDA_COUNT = BLOCK_M / G2R_A_ROWS;
    constexpr int32_t FP4_G2S_LDA_COUNT = FP4_A_GROUPS_PER_WARP * FP4_A_LD_ROUNDS_PER_GROUP;
    constexpr int32_t G2S_LDA_COUNT = FP4_A_LOAD_BY_WARP ? FP4_G2S_LDA_COUNT : LEGACY_G2S_LDA_COUNT;
    static_assert(!FP4_A_LOAD_BY_WARP || FP4_A_ROWS_PER_WARP % FP4_A_ROWS_PER_LD_ROUND == 0,
                  "FP4 A load rows must be divisible by per-round rows");
    int32_t g2r_a_col_id = threadIdx.x % G2R_A_COLS;
    int32_t g2r_a_row_id = threadIdx.x / G2R_A_COLS;
    constexpr int32_t ELEMENTS_PER_LD_B = 128 / 4 /*bit*/;
    constexpr int32_t G2R_B_COLS = COMPUTE_K / ELEMENTS_PER_LD_B;
    constexpr int32_t G2R_B_THREAD_SIZE = FP4_K_SPLIT ? 64 : BLOCK_SIZE;
    constexpr int32_t G2R_B_ROWS = G2R_B_THREAD_SIZE / G2R_B_COLS;
    static_assert((FP4_K_SPLIT && G2R_B_ROWS == WARP_N) ||
                      (!FP4_K_SPLIT && G2R_B_ROWS == BLOCK_N),
                  "unexpected B load rows");
    int32_t g2r_b_tid = FP4_K_SPLIT ? lane_id : threadIdx.x;
    int32_t g2r_b_col_id = g2r_b_tid % G2R_B_COLS;
    int32_t g2r_b_row_id_local = g2r_b_tid / G2R_B_COLS;
    int32_t g2r_b_row_id = FP4_K_SPLIT ? warp_n_id * WARP_N + g2r_b_row_id_local : g2r_b_row_id_local;
    constexpr int32_t ELEMENTS_PER_32BIT = 32 / 4;
    constexpr int32_t INT32S_PER_128BIT = 128 / 32;
    constexpr int32_t R2S_B_COLS = COMPUTE_K / ELEMENTS_PER_LD_A;

    vec8_Element<Element> a_reg[WARP_M / MFMA_M][COMPUTE_K / 32];

    if constexpr (FP4_E2M1)
    {
        if (threadIdx.x < FP4_LUT_FLOATS)
        {
            constexpr float fp4_base[FP4_LUT_FLOATS] = {
                0.0f, 0.5f, 1.0f, 1.5f, 2.0f, 3.0f, 4.0f, 6.0f,
                -0.0f, -0.5f, -1.0f, -1.5f, -2.0f, -3.0f, -4.0f, -6.0f};
            fp4_lut[threadIdx.x] = fp4_base[threadIdx.x];
        }
        __syncthreads();
    }
    ...
    for (int it_out = 0; it_out < size_k; it_out += BLOCK_K * PACK_SCALES)
    {
        ...
        uint64_t scale_offset_1 = g2r_b_row_id * (size_k / GROUP_K) + g2r_b_col_id * (size_k / GROUP_K / 4 /*hard code base on BK=64 and scale marlin each 4 interleave,fix it*/) + (size_k / GROUP_K / 2) + it_out / BLOCK_K / 2 /*increase 4 each iter*/;
        packed_scale<Element, PACK_SCALES> scale_reg;
        uint8_t scale_u8_reg[PACK_SCALES];
        if constexpr (FP4_E2M1)
        {
#pragma unroll
            for (int i = 0; i < PACK_SCALES; i++)
            {
                scale_u8_reg[i] = weight_scale_u8_ptr[g2r_b_row_id * (size_k / GROUP_K) + it_out / GROUP_K +
                                                      i * (BLOCK_K / GROUP_K) +
                                                      warp_k_id * (COMPUTE_K / GROUP_K) + g2r_b_col_id];
            }
        }
        else
        {
...
#pragma unroll
        for (int it_in = 0; it_in < PACK_SCALES * BLOCK_K; it_in += BLOCK_K)
        {
            int k_start = it_out + it_in + warp_k_id * COMPUTE_K;
            vec8_Element<Element> a_val[G2S_LDA_COUNT];
            int32_t a_lds_row[G2S_LDA_COUNT] = {0};
#pragma unroll
            for (int i = 0; i < G2S_LDA_COUNT; i++)
            {
                if constexpr (FP4_A_LOAD_BY_WARP)
                {
                    constexpr int32_t A_ROWS_PER_WARP = 16;
                    constexpr int32_t A_ROWS_PER_ROUND = 64 / G2R_A_COLS;
                    constexpr int32_t A_ROUNDS_PER_GROUP = A_ROWS_PER_WARP / A_ROWS_PER_ROUND;
                    constexpr int32_t A_LOAD_GROUPS = (BLOCK_M + A_ROWS_PER_WARP - 1) / A_ROWS_PER_WARP;
                    constexpr int32_t A_LOAD_WARPS = FP4_K_SPLIT ? (WARP_NUM / warp_k_num) : WARP_NUM;
                    constexpr int32_t A_GROUPS_PER_WARP = (A_LOAD_GROUPS + A_LOAD_WARPS - 1) / A_LOAD_WARPS;
                    static_assert(G2S_LDA_COUNT == A_GROUPS_PER_WARP * A_ROUNDS_PER_GROUP,
                                  "unexpected FP4 A load count");
                    const int32_t group_iter = i / A_ROUNDS_PER_GROUP;
                    const int32_t round_iter = i % A_ROUNDS_PER_GROUP;
                    const int32_t a_load_warp_id = FP4_K_SPLIT ? (warp_id / warp_k_num) : warp_id;
                    const int32_t a_group = a_load_warp_id + group_iter * A_LOAD_WARPS;
                    const int32_t basic_row_a =
                        a_group * A_ROWS_PER_WARP +
                        round_iter * A_ROWS_PER_ROUND +
                        (lane_id / G2R_A_COLS);
                    const int32_t basic_col_a = g2r_a_col_id;
                    a_lds_row[i] = basic_row_a;
                    if (a_group < A_LOAD_GROUPS && basic_row_a < BLOCK_M)
                    {
                        int sorted_token_idx = sorted_token_ids_offset[std::min(bidx * BLOCK_M + basic_row_a, int(sorted_token_lens - 1))];
                        int global_m = std::min(sorted_token_idx / top_k, max_n_len_offset - 1);
                        a_val[i] = *reinterpret_cast<const vec8_Element<Element> *>(
                            &input_ptr[global_m * size_k + k_start + basic_col_a * ELEMENTS_PER_LD_A]);
                    }
                }
                else
                {
            ...
            if constexpr (!FP4_E2M1)
            {
                f_scale = scale_reg[it_in / BLOCK_K];
            }
            __bf16 *p_tmp = (__bf16 *)&scale_reg;
            __bf16 bf_tmp = {};
            if constexpr (FP4_E2M1)
            ...
            vec2_fp32 scale2 = {0.0f, 0.0f};
            if constexpr (!FP4_E2M1)
            {
                scale2[0] = scale_tmp.f_data;
                scale2[1] = scale_tmp.f_data;
            }
...
#pragma unroll
            for (int i = 0; i < G2S_LDA_COUNT; i++)
            {
                int32_t basic_row_a = a_lds_row[i];
                int32_t basic_col_a = g2r_a_col_id;
                if (!FP4_A_LOAD_BY_WARP || basic_row_a < BLOCK_M)
                {
                    int swizzled_col_a = (basic_row_a + basic_col_a) % G2R_A_COLS;
                    if constexpr (!DEQUANT_16BIT)
                    {
                        *reinterpret_cast<vec8_Element<Element> *>(&lds_A[basic_row_a * COMPUTE_K + swizzled_col_a * ELEMENTS_PER_LD_A]) = a_val[i];
                    }
                    else
                    {
                        vec8_Element<Element> tmp;
                        ...
                        tmp[6] = a_val[i][3];
                        tmp[7] = a_val[i][7];
                        *reinterpret_cast<vec8_Element<Element> *>(&lds_A[basic_row_a * COMPUTE_K + swizzled_col_a * ELEMENTS_PER_LD_A]) = tmp;
                    }
                }
            }
            __syncthreads();
            for (int k_step = 0; k_step < COMPUTE_K; k_step += 32 /*4 lane * 8 bf16 per lane*/)
            {
#pragma unroll
                for (int m_tile = 0; m_tile < WARP_M / MFMA_M; m_tile++)
                {
                    ...
                    int k_idx = k_step / 8 /*cols*/ + col_id;
                    int swizzled_col_a = (m_idx + k_idx) % G2R_A_COLS;
                    a_reg[m_tile][k_step / 32] = *reinterpret_cast<vec8_Element<Element> *>(&lds_A[m_idx * COMPUTE_K + swizzled_col_a * ELEMENTS_PER_LD_A]);
                }
            }
            ...
            if constexpr (!FP4_E2M1)
            {
                scale_f2 = Dtype::num2num2(f_scale);
            }
...
#pragma unroll
            for (int i = 0; i < INT32S_PER_128BIT; i++)
            {
                if constexpr (FP4_E2M1)
                {
                    W4A16_UTILS::DEQUANTER::dequant_fp4_e2m1_e8m0_lut<Element, scalar_t2>(
                        b_val.uint_array[i], scale_u8_reg[scale_iter], fp4_lut, deq_res[i].bf162);
                }
                else if constexpr (std::is_same_v<scalar_t2, __hip_bfloat162>)
                {
...
#pragma unroll
                ...
                const vec8_Element<Element> *b_shuffled = reinterpret_cast<const vec8_Element<Element> *>(deq_res[i].bf162);
                int c = g2r_b_col_id * (ELEMENTS_PER_LD_B / ELEMENTS_PER_32BIT) + i; // 列索引
                int r = FP4_K_SPLIT ? g2r_b_row_id_local : g2r_b_row_id;
                int swizzled_col_b = (r + c) % R2S_B_COLS;
                *reinterpret_cast<vec8_Element<Element> *>(&lds_B[r * COMPUTE_K + swizzled_col_b * ELEMENTS_PER_32BIT]) = *b_shuffled;
            }
...
#pragma unroll
                for (int n_tile = 0; n_tile < WARP_N / MFMA_N; n_tile++)
                {
                    int n_idx = FP4_K_SPLIT ? n_tile * MFMA_N + row_id : warp_n_id * WARP_N + n_tile * MFMA_N + row_id;
                    int k_idx = k_step / 8 /*cols*/ + col_id;
                    int swizzled_col_b = (n_idx + k_idx) % R2S_B_COLS;
                    b_reg[n_tile] = *reinterpret_cast<vec8_Element<Element> *>(&lds_B[n_idx * COMPUTE_K + swizzled_col_b * ELEMENTS_PER_LD_A]);
                }
    ...
    const int warp_m_id)
{
    ...
    auto g_expert_scales_u8 = tcp_cache_swizzle_func<1, uint8_t>(weight_scale_u8_ptr);
    ...
    uint8_t scale_u8_reg[WARP_N / MFMA_N][SCALE_CO_LOAD_COUNT];
...
#pragma unroll
        for (int n_tile = 0; n_tile < (WARP_N / MFMA_N); n_tile++)
        {
            ...
            {
#pragma unroll
                for (int i = 0; i < SCALE_CO_LOAD_COUNT; i++)
                {
                    scale_u8_reg[n_tile][i] = g_expert_scales_u8[scale_offset_n + k_start / SCALE_RANGE + i];
                }
            }
            else
            {
...
#pragma unroll
            for (int j = 0; j < 4; j++)
            {
                if constexpr (FP4_E2M1)
                {
                    W4A16_UTILS::DEQUANTER::dequant_fp4_e2m1_e8m0<Element, scalar_t2>(
                        B_int_reg[n_tile][it_stage].uint_array[j],
                        scale_u8_reg[n_tile][col_id],
                        B_reg[n_tile][it_stage][j].bf162);
                }
                else
                {
...
#pragma unroll
    ...
    auto dequant_and_matmul = [&](int32_t it_stage, int32_t it_scale_stage)
    {
        if constexpr (std::is_same_v<scalar_t2, __hip_bfloat162>)
        {
            ...
            else if constexpr (DEQUANT_16BIT)
            {
...
#pragma unroll
    ...
    for (int it_k = 0; it_k < (size_k / (stage_offset * SCALE_CO_LOAD_COUNT) - 1 /*leave one stage remainder*/); it_k++)
    {
        first_stage(0);
#pragma unroll
